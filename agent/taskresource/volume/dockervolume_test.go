...
func TestCreateSuccess(t *testing.T) {
	...
	defer cancel()
	volume, _ := NewVolumeResource(name, name, scope, autoprovision, driver, driverOptions, nil, mockClient, ctx)
	err := volume.Create()
...
func TestCreateError(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.TODO())
	defer cancel()
	volume, _ := NewVolumeResource(name, name, scope, autoprovision, driver, nil, labels, mockClient, ctx)
	err := volume.Create()
	assert.NotNil(t, err)
}
...
func TestCleanupSuccess(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.TODO())
	defer cancel()
	volume, _ := NewVolumeResource(name, name, scope, autoprovision, driver, nil, nil, mockClient, ctx)
	err := volume.Cleanup()
	assert.NoError(t, err)
}
...
func TestApplyTransitionForTaskScopeVolume(t *testing.T) {
	...
	volume, _ := NewVolumeResource(name, name, scope, autoprovision, driver, driverOptions, labels, mockClient, nil)
...
func TestApplyTransitionForSharedScopeVolume(t *testing.T) {
	...
	volume, _ := NewVolumeResource(name, name, scope, autoprovision, driver, nil, nil, mockClient, nil)
	volume.ApplyTransition(resourcestatus.ResourceStatus(VolumeCreated))
	volume.ApplyTransition(resourcestatus.ResourceStatus(VolumeRemoved))
}
...
func TestMarshall(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.TODO())
	defer cancel()
	volume, _ := NewVolumeResource(name, name, scope, autoprovision, driver, driverOpts, labels, nil, ctx)
...
func TestNewVolumeResource(t *testing.T) {
	...
	for _, testcase := range testCases {
		t.Run(fmt.Sprintf("scope %s, autoprovision: %s", testcase.scope, testcase.autoprovision), func(t *testing.T) {
			_, err := NewVolumeResource("volume", "dockerVolume", testcase.scope, testcase.autoprovision, "", nil, nil, nil, nil)
			if testcase.fail {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
